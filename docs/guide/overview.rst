...
:doc:`Models <../api/models>`, and :doc:`Modules <../api/modules>`. In conjunction with the library's additional
components: :doc:`Environment <../api/env>`, :doc:`Storage <../api/storage>`, :doc:`Extensions <../api/extensions>`, and
:doc:`Utils <../api/utils>`, they form a complete learning pipeline. After describing each
:ref:`core component <core-components>` and :ref:`additional component <additional-components>`, we introduce the
:ref:`library's features <library-features>` and finally provide a :ref:`minimal example <example-integration>` of how
to integrate RSL-RL into a project.
...
Core components form the backbone of the library, implementing the learning loop, algorithmic logic, and neural network 
architectures.
...
:class:`~rsl_rl.models.rnn_model.RNNModel`
   ...
   recurrent network can be either an Long Short-Term Memory (LSTM) or a Gated Recurrent Unit (GRU). Its output is
   passed through the MLP to produce the final output.
