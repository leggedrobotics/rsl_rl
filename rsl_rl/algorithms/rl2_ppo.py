...
from rsl_rl.modules import ActorCriticRL2
...
class RL2PPO:
    ...
    def act(self, obs, critic_obs, prev_action):
    ...
    def update(self):  # noqa: C901
        ...
        # addtionally output prev_action_batch
        for (
            obs_batch,
            prev_actions_batch,
            critic_obs_batch,
            ...
            # -- actor
            # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
            # RL^2 addtionally input prev_actions
            self.policy.act(obs_batch, prev_actions_batch, masks=masks_batch, hidden_states=hid_states_batch[0])
            # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
            actions_log_prob_batch = self.policy.get_actions_log_prob(actions_batch)
            # -- critic
            value_batch = self.policy.evaluate(critic_obs_batch, prev_actions_batch, masks=masks_batch, hidden_states=hid_states_batch[1])
            # -- entropy
